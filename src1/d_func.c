...
/*   Updated: 2020/03/14 15:20:53 by pstein           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (reader->code[reader->i]  == 0x08)
		return (ft_d_xor(reader));
	else if (reader->code[reader->i]  == 0x09)
...
{
	if (reader->code[reader->i]  == 0x00)
		return (ft_d_live(reader));
	else if (reader->code[reader->i]  == 0x01)
...
int32_t		code_to_int(t_read *reader, size_t size)
{
	...
	result = 0;
	sign = (t_bool)(reader->code[reader->i] & 0x80);
	i = 0;
	while (size)
	{
		if (sign)
